...
bipartite_sets = nx.algorithms.bipartite.sets
...
def rank_maximal_matching(G, rank="rank", top_nodes=None):
    ...
    and the weight of the edges as the ranks.
        ...
        >>> G.add_weighted_edges_from([('a1', 'p1', 2),
        ... ('a1', 'p2', 1), ('a2', 'p2', 2)])
                    ...
                    G =   a1-----------p1
                            \
                             \
                              ...
                              2     \
                       a2-----------p2
     The matching M1 is {'a1':'p2', 'p2':'a1'}
     so O1, EV1 and U1 are {a1,p2},{a2,p1},{} respectively.
     After removing the edges incident to O1 with the rank higher than 1
     {(a1,p1),(a2,p2)} there are no more edges
     to add to G1, so an augmenting path doesn't exist,
     and the algorithm ends returning M1.

    another example:
        >>> G = nx.Graph()
        >>> G.add_nodes_from(["a1", "a2"])
        >>> G.add_nodes_from(["p1", "p2"])
        >>> G.add_weighted_edges_from([("a1", "p2", 1), ("a1", "p1", 1),
        ... ("a2", "p2", 2)])
        >>> M = rank_maximal_matching(G, rank="weight", top_nodes=["a1", "a2"])
    ...
    Notes
    -----
    This function uses the algorithm published in the article of
    Irving et al. (2006), "Rank maximal matching".
    ...
    References
    ----------
    Irving, Robert W. and Kavitha, Telikepalli and Mehlhorn, Kurt and Michail,
     Dimitrios and Paluch, Katarzyna E.,
